...
/**
 * Build a Socket Hook Schema Object and/or Parameter Object from LoopBack
 * type descriptor.
 *
 ...
 * @returns {Object} Socket Hook Schema Object that can be used as `schema` field
...
/**
 * Convert validations and other metadata from LDL format to SocketHook format.
 * @param {Object} ldlDef LDL property/argument definition,
 * for example `{ type: 'string', maxLength: 64 }`.
 * @return {Object} Metadata in SocketHook format.
 */
exports.buildMetadata = function(ldlDef) {
  ...
  for (var ix in SOCKET_HOOK_TYPE_FIELDS) {
    key = SOCKET_HOOK_TYPE_FIELDS[ix];
    if (key in ldlDef)
      result[key] = ldlDef[key];
  }

  if (ldlDef.description) {
    result.description = textConverter.convertText(ldlDef.description);
  } else if (ldlDef.doc) {
    result.description = textConverter.convertText(ldlDef.doc);
  }
